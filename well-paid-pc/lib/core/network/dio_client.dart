...
String? messageFromDio(Object error, [AppLocalizations? l10n]) {
  ...
  switch (error.type) {
    ...
    case DioExceptionType.connectionError:
      final baseHint = l10n?.dioConnectionFailed ??
          'Sem ligação ao servidor. Confirma o URL da API e que o backend responde.';
      if (kIsWeb) {
        return '$baseHint\n'
            'URL actual: ${ApiConfig.baseUrl}\n'
            'Na web, falhas comuns: CORS no deploy (na Vercel usa CORS_ORIGINS=* ou inclui '
            'http://localhost:PORT), extensão a bloquear pedidos, ou build antigo — '
            'corre flutter clean e volta a fazer flutter run -d chrome.';
      }
      return '$baseHint\n'
          'URL actual: ${ApiConfig.baseUrl}\n'
          'Backend local: flutter run --dart-define=API_BASE_URL=http://127.0.0.1:8000';
