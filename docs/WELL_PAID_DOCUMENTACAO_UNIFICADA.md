...
**APK sideload sem site próprio (GitHub Releases + API):** a app consulta `{API_BASE_URL}/android-update.json` (origem versionada em [`backend/app/static/android-update.json`](../backend/app/static/android-update.json)). O campo `apk_url` deve ser um link **HTTPS** para o ficheiro `.apk` (por exemplo o URL de download de um asset numa **Release** do GitHub). O workflow CI [`.github/workflows/android-apk-release.yml`](../.github/workflows/android-apk-release.yml) compila `assembleRelease` (runner `windows-latest`, `gradlew.bat`) e publica `wellpaid-<tag>.apk` ao fazer **push** de uma tag `v*` ou ao correr **Actions → Android APK release** com o input `release_tag`. Segredos opcionais de assinatura: ver comentários no YAML. Para os preencher a partir do teu `keystore.properties` local, corre na máquina onde tens o keystore: [`android-native/scripts/Export-GithubAndroidSecrets.ps1`](../android-native/scripts/Export-GithubAndroidSecrets.ps1). Se tiveres [GitHub CLI](https://cli.github.com/) instalado e sessão (`gh auth login`), o script usa `gh secret set` no repo; senão copia os valores para a área de transferência. Opcional: `-Repo Dono/repo` ou `-ClipboardOnly`. **Tutorial passo-a-passo:** [`docs/Diretrizes Well Paid/Diretrizes Well Paid.md`](./Diretrizes%20Well%20Paid/Diretrizes%20Well%20Paid.md).
